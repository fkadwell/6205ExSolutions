...
public class _1431_KidsWithTheGreatestNumberOfCandies {
  ...
  public static void main(String[] args) {
    Solution solution = new _1431_KidsWithTheGreatestNumberOfCandies().new Solution();
    int[] a = {2, 3, 5, 1, 3};
    int etr = 3;
    System.out.println(solution.kidsWithCandies(a, etr));
  }
  ...
  class Solution {
    ...
    public List<Boolean> kidsWithCandies(int[] candies, int extraCandies) {
      ...
      int[] diff = getDiff(candies, max);
      List<Boolean> res = new ArrayList<>();
      for (int value : diff) {
        if (value + extraCandies >= 0) res.add(Boolean.TRUE);
        else res.add(Boolean.FALSE);
      }
      return res;
    }

    private int[] getDiff(int[] candies, int max) {
